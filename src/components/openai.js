...
export default function OpenAI() {
  ...
  async function onSubmitext(event) {
    ...
    try {
      const response = await ServiceDavinci003.getDaVinci({
        animal: animalInputText,
      });
  ...
  async function onSubmitra(event) {
    event.preventDefault();
    try {
      const response = await ServiceTra.getTraduccion({
        animal: animalInputTra,
      });
      const data = await response;
      console.log(response);
      if (response.status !== 200) {
  ...
  return (
    <div style={{ marginTop: "9rem" }}>
      ...
      <button
        className="pinterest-btn"
        onClick={() => setSelectedService("text")}
      >
        {t("generator of characters")}
      </button>
      <button
        className="pinterest-btn"
        onClick={() => setSelectedService("emo")}
      >
        {t("generator of emojis")}
      </button>
      <button
        className="pinterest-btn"
        onClick={() => setSelectedService("tra")}
      >
        {t("music name translator")}
      </button>
      <button
        className="pinterest-btn"
        onClick={() => setSelectedService("list")}
      >
        {t("list of music")}
      </button>
      <button
        className="pinterest-btn"
        onClick={() => setSelectedService("clas")}
      >
        {t("list of music genres")}
      </button>
            ...
            <form onSubmit={onSubmitemo}>
              ...
              <div className="cont-center">
                <input
                  type="submit"
                  className="pinterest-btn pinterest-btn--black"
                  value={t("generate emojis")}
                />
              </div>
            </form>
            <br />
            <div style={{ color: "white" }} className="text-center fly">
              {resultemo}
            </div>
            {user && (
              <SavePrompt
